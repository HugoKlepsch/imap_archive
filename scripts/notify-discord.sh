...
#   ./scripts/notify-discord.sh --failure imap-archive-backup.service
#   ./scripts/notify-discord.sh --success imap-archive-check.service "5% verified"
#
# Invoked by imap-archive-alert@.service via OnFailure= on the units that
...
case "$MODE" in
  ...
  --test)
    ...
    COLOUR=$BLUE
    DETAIL="If you can read this, alerting from the IMAP archive works."
    STATE="test"
    UNIT="imap-archive"
    ;;
  *)
